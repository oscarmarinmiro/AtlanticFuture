...
MOVEMENTS_WORLD_OUT = "../data/movements_world_network.tsv"
...
def insertSourceTarget(year,source, target, value, struct):
    ...
    if target not in struct[year][source]:
        ...
        if source == NON_ATLANTIC or target == NON_ATLANTIC:
            if 'HDI' in atlanticISO2[source] and 'HDI' in atlanticISO2[target]:
                struct[year][source][target]['value']+= value
...
for row in reader:
    ...
    for i in range(1,len(row)):
        ...
        if NON_ATLANTIC not in distanceMatrix:
            distanceMatrix[NON_ATLANTIC] = {}
        distanceMatrix[source][target] = distance
        distanceMatrix[source][NON_ATLANTIC] = AVERAGE_DISTANCE
        distanceMatrix[NON_ATLANTIC][target] = AVERAGE_DISTANCE
...
# Artificial non-atlantic node

atlanticISO2[NON_ATLANTIC] = {'HDI': 0.64,'M49':999999, 'currencyName': 'Unknown','currencyNumeric': -1, 'isIndependent': 'Unknown', 'iso2': NON_ATLANTIC, 'lat': '0.0','lon': '0.0','name': 'Non-atlantic','pop2005': 6454000000 - 1236535089}
        ...
        if country in extraData and 'Country code' in extraData[country]:
            ...
            for countryData in migrationData['IN']:
                ...
                if thisISO2 in atlanticISO2:
                    ...
                    if myISO2!= NON_ATLANTIC:
                        insertSourceTarget(year,myISO2, NON_ATLANTIC,countryData['value'],countryMovements)
            ...
            for countryData in migrationData['OUT']:
                ...
                if thisISO2 in atlanticISO2:
                    ...
                    if myISO2!= NON_ATLANTIC:
                        insertSourceTarget(year, NON_ATLANTIC, myISO2, countryData['value'],countryMovements)
...
fileOut = open(MOVEMENTS_WORLD_OUT,"wb")
